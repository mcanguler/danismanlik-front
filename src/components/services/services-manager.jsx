...
function ServiceAvatar({ service }) {
  return (
    <Avatar className="size-8 rounded-lg after:rounded-lg">
      <AvatarImage src={service.image} alt={service.name} className="rounded-lg" />
...
export function ServicesManager() {
  ...
  const handleDelete = () => {
    ...
    deleteMutation.mutate(deleting.id, {
      onSuccess: () => {
        toast.add({ title: "Hizmet silindi", type: "success" });
        setDeleting(null);
      },
      onError: (error) => {
        toast.add({
      ...
      <div className="mt-4">
        ...
        {query.isSuccess && services.length === 0 && (
          <div className="flex flex-col items-center gap-3 rounded-xl border px-4 py-14 text-center">
            ...
            <p className="text-sm font-medium">Henüz hizmet yok</p>
            <p className="text-sm text-muted-foreground">
